...
== Exploring minitest, October 2013
...
- executing the initial tests with minitest:
  - rake test                # Run tests
  - rake test:clean          # Executes the 'test' task after deleting the previous coverage result files

- implementation notes
  - successfully got minitest to work with simplecov!
  - added 'minitest' to the Gemfile
  - added file 'test/minitest_helper.rb', and required it in 'test/age_test.rb'
  - added these two tasks to rakefile.rb
  ...
  - the 'rake test' and 'rake test:clean' tasks launch the tests in another process,
    which is where SimpleCov is loaded and used (not in the rakefile main process).
  - Added some process id logging; this can be removed.
